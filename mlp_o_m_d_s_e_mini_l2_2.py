...
class HiddenLayer(object):
       ...
       def backward(self, delta, lamdb):
              ...
              mode = dropout_param['mode']
              #print('input.shape in bp', self.input.shape)
              #print('mask.shape in bp = ', mask.shape)
              #print('mask * input in bp size = ', (mask * self.input).shape)
              if mode == 'train':
...
class MLP:
       ...
       def regulation_cost(self, lambd, mini_batch_size):
           ...
           regularization_cost = lambd * np.sum(l_W) / mini_batch_size /2
       ...
       def fit(self, X, y, learning_rate=0.1, epsilon=0.01 ,epochs=100, lamdb=0.01, momemtum_rate=0.5, dropout_param={'p':0.5 ,'mode':'train','seed':6},mini_batch_size=64):
            ...
            while True:
                for k in range(epochs):
                        ...
                        for minibatch in minibatches:
                            ...
                            mini_count += 1
                            #regulation_cost = self.regulation_cost(lamdb, mini_batch_size)
                            #average_loss += regulation_cost
                        # update
                        ...
                        print('epochs = ', k, 'loss = ', average_loss/X.shape[0], 'duration = ', epochs_end_time-epochs_start_time)
                                #print("layer ",i, "size = ", self.layers[i].W.shape)
                to_return_last = (average_loss) / X.shape[0]
                if to_return_last - to_return < epsilon:
                    break
                else:
                    to_return = to_return_last
...
MSE = nn.fit(input_data, output_data, learning_rate=0.01, epsilon=0.0001, epochs=5, lamdb=0.5, momemtum_rate=0.99,dropout_param={'p':0.4 ,'mode':'train','seed':5},mini_batch_size=64)
