...
data_sample = np.zeros([128, data.shape[1]])
data_sample_label = np.zeros(128)
i = 0
while i < 128:
...
class HiddenLayer(object):
       ...
       def dropout_forward(self, x, dropout_param):
           ...
           if mode == 'train':
               ...
               mask = (np.random.rand(*self.W.shape) < p) / p
               out = x
       ...
       def forward(self, input, dropout_param):
              ...
              if mode == 'train':
                return self.output, mask * self.W
              elif mode == 'test':
...
class MLP:
       ...
       def loss(self, X, y):
           ...
           #print('y = ', y)
           #print('probs = ',probs)
           correct_logprobs = 0.0
           for i in range(num_examples):
                correct_logprobs += -np.log(probs[i, np.int(y[i])])
           #print('correct_logprobs = ',correct_logprobs)
           #data_loss = np.sum(correct_logprobs)
           return correct_logprobs / num_examples
       ...
       def fit(self, X, y, learning_rate=0.1, epochs=100, lamdb=0.01, momemtum_rate=0.5, dropout_param={'p':0.5 ,'mode':'train','seed':6},mini_batch_size=64):
            ...
            for k in range(epochs):
                    ...
                    for minibatch in minibatches:
                        ...
                        loss = 0.0
                        print('mini_batch_X size = ', minibatch_X.shape)
                        print('mini_batch_Y size = ', minibatch_y.shape)
                        y_hat = self.forward(minibatch_X, dropout_param)
                        delta = self.diff(y_hat, minibatch_y)
                        print('delta.shape', delta.shape)
                        self.backward(delta, lamdb)
                        print('y_hat shape', y_hat.shape)
                        loss = self.criterion_MSE(minibatch_y, y_hat, lamdb)
                        break
...
MSE = nn.fit(input_data, output_data, learning_rate=0.01, epochs=5, lamdb=0.5, momemtum_rate=0.99,dropout_param={'p':0.4 ,'mode':'train','seed':5},mini_batch_size=64)
