...
class Activation(object):
    ...
    def __relu_deriv(self, a):
        ...
        dx[a <= 0] = 0
        return dx
    def __reluleaky(self, x):
        return np.maximum(0.001 * x, 0.05 * x)

    def __reluleaky_deriv(self, a):
        dx = np.ones_like(a)
        dx[a <= 0] = 0.001
        return dx
    ...
    def __init__(self,activation='relu'):
        ...
        elif activation == 'reluleaky':
            self.f = self.__reluleaky
            self.f_deriv = self.__reluleaky_deriv
...
class MLP:
       ...
       def __init__(self, layers, activation='relu'):
              ...
              for i in range(len(layers)-3):  # equip the layer
                     ...
                     self.layers.append(HiddenLayer(layers[i], layers[i + 1], activation='reluleaky',  num_layer=i))
                     self.layers.append(HiddenLayer(layers[i+1], layers[i + 2], activation='reluleaky', num_layer=i))
...
nn = MLP([128,90,30,10], 'relu')     # auto initialize three class: MLP, HiddenLayer, Activation
input_data = data_sample
output_data = data_sample_label
### Try different learning rate and epochs

MSE = nn.fit(input_data, output_data, learning_rate=0.001, epochs=2, lamdb=0.5, momemtum_rate=0.99,dropout_param={'p':0.5 ,'mode':'train','seed':6})
